...
void init_fast(){
	...
	*/
    ...
    for(int e=0;e<e_num;e++){//ÒòÎª³õÊ¼»¯Ê±´ÓÒ»Ìõ±ßµÄÁ½¸ö¶¥µãÖÐÑ¡Ò»¸ö£¬ËùÒÔÒ»¶¨Ã»ÓÐscore+=weightµÄÇé¿ö
        v1=e_vertex[e][0];
        v2=e_vertex[e][1];
...
void add(int c){
	...
	for(h=0;h<nrow[c]; h++){//Cé›†åˆä¸­æ¯ä¸€ä¸ªå˜é‡h   å¤„ç†æœªè¦†ç›–é›†å�?
		...
		cnt=0;
		for(l=0;l<2;l++){//å› ä¸ºè¿™ä¸ªå˜é‡ï¼Œå˜æˆè¿™ä¸ªé›†åˆçš„é‚»å±…é›†å�?
			j=e_vertex[i][l];
			if(j==c) continue;
			if(cs[j].is_in_c){
				s=j;
...
void localsearch(int maxStep){
	...
	while(step<=maxStep){
		if(uncover_num==0){
			int k=cost_C();
			if(k<best_value){update_best_sol();NoImpro=0;}
			else NoImpro++;
			if(best_value==BEST){
		...
		if(NumRemove>1&&NoImpro<=100){NumRemove--;NoImpro=0;}
		...
		while(uncover_num>0){
			...
			for(h=0; h<2;h++){
				...
				if(cs[j].config==0){
					...
					if(cs[j].cost+cost>=best_value) continue;
					int j_uncover_edges=0;
					for(int ii=0;ii<cs[j].degree;ii++){
						int ee=v_edges[j][ii];
						if(index_uncover[ee]!=-1) j_uncover_edges++;
					}
					if(j_uncover_edges==uncover_num){maxc=j;}
...
int main(int argc, char *argv[]){
	...
	if(argc<2){
		printf("usage ./msp instance_path rand_seed time_limit\n");
		return 0;
	}
